...
function AIGeneratedQuestions({ 
    ...
    // Fonction pour générer des questions avec l'IA (avec vérification de disponibilité)
    const handleGenerateQuestions = async (): Promise<void> => {
        ...
        try {
            // Utiliser la nouvelle méthode qui vérifie la disponibilité et affiche des toasts
            const result = await AIService.generateQuestionsWithAvailabilityCheck(
                inputText, 
                numberOfQuestions,
                showToastMessage // Callback pour afficher les toasts
            );
    ...
    const handleAddGeneratedQuestions = async (): Promise<void> => {
        ...
        try {
            ...
            // Vider les questions générées après ajout
            setGeneratedQuestions([]);
            setInputText('');
